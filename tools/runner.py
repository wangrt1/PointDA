    ...
    with torch.no_grad():
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN' or dataset_name == 'Projected_ShapeNet':
                ...
                coarse_points = ret[0]
                dense_points = ret[-1]
                print("coarse_points: ",type(coarse_points))
                print("coarse_points: ",coarse_points.size())
                coarse_points_ = coarse_points.cpu().numpy()
                dense_points_ = dense_points.cpu().numpy()
                gt_ = gt.cpu().numpy()
                partial_ = partial.cpu().numpy()
                point_cloud_c = open3d.geometry.PointCloud()
                point_cloud_d = open3d.geometry.PointCloud()
                point_cloud_g = open3d.geometry.PointCloud()
                point_cloud_p = open3d.geometry.PointCloud()
                point_cloud_c.points = open3d.utility.Vector3dVector(coarse_points_[0,:,:])
                point_cloud_d.points = open3d.utility.Vector3dVector(dense_points_[0,:,:])
                point_cloud_g.points = open3d.utility.Vector3dVector(gt_[0,:,:])
                point_cloud_p.points = open3d.utility.Vector3dVector(partial_[0,:,:])
                if not os.path.exists(f"temp_set/{taxonomy_id}"):
                    os.makedirs(f"temp_set/{taxonomy_id}/coarse_points")
                    os.makedirs(f"temp_set/{taxonomy_id}/dense_points")
                    os.makedirs(f"temp_set/{taxonomy_id}/ground_truth")
                    os.makedirs(f"temp_set/{taxonomy_id}/partial_points")
                open3d.io.write_point_cloud(f"temp_set/{taxonomy_id}/coarse_points/{model_id}.pcd",point_cloud_c)
                open3d.io.write_point_cloud(f"temp_set/{taxonomy_id}/dense_points/{model_id}.pcd",point_cloud_d)
                open3d.io.write_point_cloud(f"temp_set/{taxonomy_id}/ground_truth/{model_id}.pcd",point_cloud_g)
                open3d.io.write_point_cloud(f"temp_set/{taxonomy_id}/partial_points/{model_id}.pcd",point_cloud_p)
